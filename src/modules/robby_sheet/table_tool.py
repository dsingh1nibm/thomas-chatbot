...
class PandasAgent :
    ...
    def get_agent_response(self, uploaded_file_content, query):
        ...
        llm = OpenAI(openai_api_key=user_api_key, temperature=0.0, model="gpt-3.5-turbo")
        df = SmartDataframe(uploaded_file_content, config={"llm": llm})
        # pandas_ai = PandasAI(llm)
        # pd_agent = create_pandas_dataframe_agent(OpenAI(temperature=0), uploaded_file_content, verbose=True)
        old_stdout = sys.stdout
        captured_output = StringIO()
        sys.stdout = captured_output
        response= df.chat(query)
        # captured_outputfinal=captured_output.getvalue()
